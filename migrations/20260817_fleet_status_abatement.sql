-- Add Abatement as a 4th railcars.fleet_status value.
-- Abatement = still leased (lease has not ended); rent is temporarily paused.
-- KPI math treats Abatement as Leased (Total Fleet / Active Assignments unchanged
-- when a Leased car is marked Abatement). abatement is a separate reportable count.
-- Safe to re-run. No backfill — existing rows stay as they are.
...
ALTER TABLE public.railcars
  ...
  CHECK (fleet_status IN ('Leased', 'Idle', 'Sold', 'Abatement'));
...
k AS (
  SELECT
    ...
    (SELECT count(*) FROM op WHERE entity = 'Main' AND fleet_status IN ('Leased', 'Abatement')) AS owned_assigned,
