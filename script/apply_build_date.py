"""Apply migrations/20260816_build_date.sql via SUPABASE_DB_PASSWORD."""
...
sql = (ROOT / "migrations" / "20260816_build_date.sql").read_text(encoding="utf-8")
...
if conn is None:
    raise SystemExit(
        "Could not connect to Postgres. Run migrations/20260816_build_date.sql in the Supabase SQL editor. "
...
try:
    with conn.cursor() as cur:
        for part in parts:
            cur.execute(part)
    print(f"Applied migrations/20260816_build_date.sql (conn #{used})")
finally:
    conn.close()
